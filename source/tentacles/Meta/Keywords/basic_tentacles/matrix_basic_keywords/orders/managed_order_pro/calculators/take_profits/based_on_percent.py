...
def calculate_take_profit_based_on_percent(
    maker,
    ...
    profit_in_d = (
        entry_price * (1 + (take_profit_settings.tp_in_p / 100))
        if entry_side == trading_enums.TradeOrderSide.BUY.value
        else entry_price
        * (1 - (take_profit_settings.tp_in_p / 100))
    )
    return profit_in_d


async def place_take_profit_based_on_percent(
